...
et
\begin{equation}
  d_{\mathit{pw}}' = \frac{1}{2}\left( \sum_s{p_{s|1}\bar{d_{s,2}}} + \sum_t{p_{t|2}\bar{d_{t,1}}} \right), 
\end{equation}
...
\begin{equation}
  ...
  = 2 H_{\Delta}\left(\frac{\vectorsym{p}_{i}+\vectorsym{p}_j}{2}\right)
  -H_{\Delta}\left(\vectorsym{p}_i\right)-H_{\Delta}\left(\vectorsym{p}_j\right).
\end{equation}
...
Les espèces peuvent être placées dans un espace multidimensionnel construit par une Analyse en Coordonnées Principales \autocite{Gower1966} de la matrice de dissimilarité $\matrixsym{D}$ telle que $\matrixsym{\Delta}=\dfrac{\matrixsym{D}^{\circ2}}{2}$ (les distances dans l'espace multidimensionnel sont $\sqrt{2\delta_{s's''}}$; autrement dit: la dissimilarité entre deux espèces est la moitié du carré de la distance entre elles dans la représentation géométrique).
