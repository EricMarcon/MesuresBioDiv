...
{Diversité $\beta$ liée au remplacement des espèces le long d'un gradient. 
  $\Delta{y}$ est la mesure de la dissimilarité entre deux communautés, voir page~\pageref{sec:BetaPaires}.
...
De nombreuses mesures de dissimilarité existent, elles sont détaillées dans le chapitre~\ref{chap:DivPaires}, à partir de la page~\pageref{sec:BetaPaires}.
...
Les indices les plus simples et probablement les plus connus pour calculer la diversité $\beta$ sont l'indice de \textcite{Jaccard1901} et de Sørensen \autocite{Dice1945, Sorensen1948}. 
...
\subsection{Emboîtement et substitution}
...
\begin{equation}
  ...
  - \frac{\min(s_{1,0},s_{0,1})}{s_{1,1}+\min(s_{1,0},s_{0,1})}
\end{equation}
...
Les mesures de diversité $\beta$ fondées sur l'entropie HCDT (chapitre \ref{chap:Beta}) ne permettent pas de distinguer directement emboîtement et substitution.
...
La distance la plus évidente entre communautés quand les effectifs des espèces sont connus est simplement la distance euclidienne (Figure~\ref{BCIPCAFig}), éventuellement normalisée par le nombre d'espèces (Dissimilarité $D_1$ de \textcite{Gower1986}).
...
{Représentation des deux premiers axes factoriels de l'ACP des 50 carrés de BCI. 
  Les données sont les nombres d'individus par espèce, ni centrées ni réduites pour l'ACP. 
...
\section{Distance entre paires de communautés et diversité $\beta$}
...
Pour un certain nombre de distances, la transformation de $p_{s|i}$ en $y_{s|i}$ est connue \autocite{Legendre2001}.
...
\lettrine{Ce}{chapitre} traite de la diversité proportionnelle, qui mesure à quel point les communautés diffèrent de leur assemblage.
...
La distance entre communautés est donc interprétable directement comme une mesure de diversité $\beta$ \autocite{Ricotta2015}, dans la tradition de l'utilisation de la dissimilarité entre paires de communautés (chapitre~\ref{chap:DivPaires}).
...
\textcite{Hardy2008} montrent que les deux pondérations sont valides mais l'absence de cadre général assurant que la diversité $\gamma$ de Rao est supérieure à la diversité $\alpha$ \autocite{DeBello2010} motive \textcite{Guiasu2011} à proposer une alternative à l'indice de Rao, l'indice de Gini-Simpson quadratique pondéré, dont la concavité est démontrée (ce qui implique que $\gamma\ge \alpha$). 
Il sera traité dans le chapitre consacré à la diversité de paires (page~\pageref{chap:DivPaires}). 
