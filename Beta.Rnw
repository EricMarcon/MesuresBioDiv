...
<<Beta-parent, echo=FALSE, cache=FALSE>>=
...
\lettrine{L}{a notion} de diversité $\beta$ a été introduite par \textcite[][page 320]{Whittaker1960} comme le niveau de changement dans la composition des communautés, ou le degré de différenciation des communautés, en relation avec les changements de milieu.
...
Le Tableau~\ref{table:Notations} résume les notations.
...
Les données sont organisées dans le package sous la forme d'un objet \code{MetaCommunity} qui contient notamment la matrice des $n_{s,i}$. 
Les parcelles de Paracou sont dans l'objet \code{Paracou618.MC}.

<<Paracou618.MC>>=
summary(Paracou618.MC)
@

\tableFW{table:Notations}
{Notations des effectifs, tableau espèces-communautés.}
{
  \begin{tabularx}{\textwidth}{p{2cm} p{7cm} p{1cm} X}
    ...
    \midrule
    Espèce $s$ 
    & $n_{s,i}$: nombre d'individus de l'espèce $s$ dans la communauté $i$. \newline
    $\hat{p}_{s|i}=n_{s,i}/n_{+i}$ est l'estimateur de la probabilité $p_{s|i}$ qu'un individu de la communauté $i$ soit de l'espèce $s$.
    &
    & $n_{s+}=\sum_i{n_{s,i}}$ \newline
...
\section[Définition de la diversité beta par objectifs]{Définition de la diversité $\beta$ par objectifs}
...
Deux types de diversité $\beta$ peuvent être définies \autocite{Vellend2001} en accord avec les notions originales de Whittaker: le \emph{remplacement} (\foreignlanguage{english}{\emph{turnover}}) des espèces le long d'un gradient environnemental, spatial ou temporel et la \emph{variation} de la composition spécifique entre communautés, qui diffèrent de la méta-communauté et entre elles.
Ces deux définitions sont présentées sur les figures~\ref{Anderson2011a} et \ref{Anderson2011b}.

\figureSC{Anderson2011a}
{Diversité $\beta$ liée au remplacement des espèces le long d'un gradient. 
  $\Delta{y}$ est la mesure de la dissimilarité entre deux communautés, voir page~\pageref{chap:BetaPaires}.
  $\vectorsym{x}$ est un gradient environnemental, spatial ou temporel.
  $\Delta{x}$ est l'écart entre deux points du gradient.
  (\emph{in} \textcite{Anderson2011})}
{\includegraphics[width=\maxwidth]{Anderson2011a}}

\figureSC{Anderson2011b}
{Diversité $\beta$ liée à la variation de la composition spécifique.
  $\beta_W$ est le rapport entre la diversité $\gamma$ et la diversité $\alpha$, $\beta_{Add}$ leur différence.
  $d_{ij}$ est la dissimilarité entre les communautés $i$ et $j$.
  $\hat{\sigma}^2$ est la variance de cette dissimilarité, $\bar{d}$ sa moyenne.
  Si $d$ est une distance euclidienne, $\bar{d}_{cen}$ est la distance moyenne des communautés à leur centre de gravité.
  (\emph{in} \textcite{Anderson2011})}
{\includegraphics[width=\maxwidth]{Anderson2011b}}

Les mesures de remplacement ne sont pas traitées en détail dans ce chapitre mais simplement présentées ici.

La plus simple (Figure~\ref{Anderson2011a}, T1) est la dissimilarité entre deux communautés notée $\Delta{y}$.
De nombreuses mesures de dissimilarité existent, elles sont détaillées dans le chapitre~\ref{chap:BetaPaires}, à partir de la page~\pageref{chap:BetaPaires}.
...
La dissimilarité entre des paires de communautés présentant la même différence (par exemple ayant subi ou non un traitement) situées le long d'un gradient (par exemple la fertilité du sol) peut être analysée en fonction de ce gradient (Figure~\ref{Anderson2011a}, T2).
...
Une variation sur le modèle T2 consiste à le représenter sous la forme d'un modèle de décroissance en fonction de la distance \autocite{Nekola1999}, éventuellement pour plusieurs groupes de communautés (Figure~\ref{Anderson2011a}, T4 et T5). 
L'indice de Simpson spatialement explicite (page~\pageref{sec:SimpsonSpatial}) entre dans ce cadre bien qu'il soit construit différemment.

Les mesures de variation (Figure~\ref{Anderson2011b}, V1) concernent la dissimilarité entre plusieurs communautés ou entre chaque communauté et la méta-communauté.
L'équivalence de ces deux concepts n'est pas évidente: elle a été montrée pour les mesures de dissimilarité entre paires de communautés par \textcite{Legendre1999, Legendre2001, Legendre2005}, pour l'entropie de Rao par \textcite{Pavoine2004} et pour l'entropie HCDT par \textcite{Marcon2014a}.
...
Cette dernière approche est traitée dans ce chapitre, où il sera montré que la mesure dérivée est aussi une dissimilarité moyenne entre communautés.

Si la diversité est mesurée comme moyenne des dissimilarités, elle peut être analysée par des méthodes d'ordination indirectes (Figure~\ref{Anderson2011b}, V2) ou directes (V3). Elle peut aussi être comparée entre méta-communautés ou modélisée en fonction d'un gradient environnemental (V4), de façon similaire au modèle T6.
...
\begin{itemize}
  \item  La diversité d'inventaire (\emph{inventory diversity}), qui traite des données récoltées sur une unité spatiale, ce qui correspond à la définition des diversités$\ \alpha$ et $\gamma$;
  \item  La diversité de différenciation (\emph{differentiation diversity}), qui mesure à quel point les unités spatiales sont différentes, ce qui correspond à la définition littérale de la diversité $\beta$ de Whittaker;
  \item  La diversité proportionnelle (\emph{proportional diversity}), diversité $\beta$ qui se construit par différence ou rapport des diversités $\gamma$ et $\alpha$. \textcite{Marcon2014a} ont montré qu'avec une décomposition appropriée de la diversité, la diversité proportionnelle est aussi la divergence entre la distribution des communautés locales et celle de la méta-communauté: la diversité $\beta$ mesure alors à quel point les unités spatiales sont différentes de leur assemblage plutôt qu'entre elles.
\end{itemize}

Dans son article fondateur, \textcite{Whittaker1960} définit la diversité $\beta$ comme le niveau de changement dans la composition des communautés, la nomme \og diversité secondaire \fg{} et lui attribue la lettre $\beta$ en référence à la diversité \og primaire \fg{}, mesurée par l'indice $alpha$ de Fisher \autocite{Fisher1943} (voir le chapitre \ref{chap:Fisher}, page \pageref{chap:Fisher}).
...
\section{Décomposition de la diversité}
...
\subsection[Définitions de la diversité beta, mesure dérivée]{Définitions de la diversité $\beta$, mesure dérivée}
...
\begin{itemize}
  \item  Une définition de la mesure de diversité, appliquée à la diversité $\gamma$, qui est généralement une des mesures vues dans le chapitre~\ref{chap:MesuresNeutres};
  \item  Une définition de la diversité $\alpha$, qui peut être par exemple:
  \begin{itemize}
    \item  La diversité locale mesurée dans chaque communauté, indépendamment de toute référence hors de la communauté;
    \item  De façon équivalente, le nombre d'espèces effectives dans les communautés.
  \end{itemize}
  \item  Une façon de combiner les diversités $\gamma$ et $\alpha$ pour obtenir la diversité $\beta$, par exemple:
  \begin{itemize}
...
L'utilisation des nombres de Hill, la mesure locale de la diversité $\alpha$ et la définition de la diversité $\beta$ comme rapport des diversités $\gamma$ et $\alpha$ permet de définir la \og vraie diversité\fg{} \autocite{Jost2006, Jost2007} $\beta$ qui est un nombre de communautés équivalentes (\foreignlanguage{english}{\emph{compositionnal units}}) similaire au nombre d'espèces équivalentes des diversités $\alpha$ et $\gamma$. 
\textcite{Tuomisto2011} milite pour que le terme diversité soit réservé à la vraie diversité (homogène à un nombre d'espèces) et que les autres mesures soient appelées différemment: \og entropie\fg{} de Shannon ou \og probabilité\fg{} de Gini-Simpson notamment.

\subsection{Le débat sur la décomposition}
\label{subseq:DebatDecomposition}
...
\textcite{Whittaker1960, Whittaker1972} est l'auteur de ce concept. 
...
Il note que \og la partition serait plus facilement interprétable si les différentes composantes de la diversité pouvaient être exprimés au moyen de la même formule\fg{} (ce qui n'est en fait jamais le cas). 
...
\subsection{Décomposition multiplicative de la diversité}
...
\begin{equation}
  \xq{D}{q}{}{\gamma}{} = \xq[\,]{D}{q}{}{\alpha}{} \xq[\,]{D}{q}{}{\beta}{}.
  \label{eq:DecompDq}
\end{equation}

$\xq{D}{q}{}{\alpha}{}$ et $\xq{D}{q}{}{\gamma}{}$ sont les nombres de Hill d'ordre $q$ égaux aux diversités $\alpha$ et $\gamma$. 
Ce sont des nombres équivalents d'espèces. 
$\xq{D}{q}{}{\beta}{}$ est le \og nombre de communautés effectives\fg{} ou \og nombre équivalent de communautés \fg{}, c'est-à-dire le nombre de communautés de poids égal ne possédant aucune espèce en commun (et dont la dissimilarité entre les espèces de communautés différentes est maximale quand la diversité est phylogénétique ou fonctionnelle) \autocite{Pavoine2015a}, qui fourniraient la même valeur de diversité $\beta$.

La diversité $\beta$ est indépendante de la diversité $\alpha$ si les poids des communautés sont égaux. 
L'\emph{indépendance} signifie que la valeur de $\xq{D}{q}{}{\beta}{}$ n'est pas contrainte par celle de $\xq{D}{q}{}{\alpha}{}$. 
Cette propriété est souvent considérée comme importante \autocite{Wilson1984, Gregorius2010}, et sera discutée largement ici.


\subsection[Définitions de la diversité alpha]{Définitions de la diversité $\alpha$}
\label{sec:defalpha}

La diversité $\alpha$ est calculée pour chaque communauté, et notée $^q_iD_{\alpha}$ (l'entropie correspondante est notée $\xq{H}{q}{}{\alpha}{i}$). 
...
\begin{equation}
  \xq{H}{q}{}{\alpha}{} = \sum_i{w_i\xq[\,]{H}{q}{}{\alpha}{i}};
\end{equation}

\begin{equation}
  \xq{D}{q}{}{\alpha}{}={\left(\sum_s{\sum_i{w_ip^q_{s|i}}}\right)}^{1/{\left(1-q\right)}}.
\end{equation}
...
\begin{equation}
  \xq{H}{q}{}{\alpha}{} = \sum_i{\frac{w^q_i}{\sum_i{w^q_i}}\xq[\,]{H}{q}{}{\alpha}{i}};
\end{equation}

\begin{equation}
  \xq{D}{q}{}{\alpha}{} = {\left(\sum_s{\sum_i{\frac{w^q_i}{\sum_i{w^q_i}}p^q_{s|i}}}\right)}^{{1}/{\left(1-q\right)}}.
\end{equation}
...
\begin{equation}
  \xq{D}{q}{}{\alpha}{}=\frac{1}{I}{\left(\sum_s{\sum_i{{\left(w_ip_{s|i}\right)}^q}}\right)}^{{1}/{\left(1-q\right)}}.
\end{equation}
...
Cette question est traitée en détail page~\pageref{sec:DiversiteJointe}.
...
La dépendance entre $\xq{D}{q}{}{\beta}{}$ et $\xq{D}{q}{}{\alpha}{}$ est le prix à payer.



\chapter{Diversité de différentiation}
\label{chap:BetaPaires}
...
\lettrine{L}{a littérature} des indices de similarité est bien antérieure à celle de la biodiversité.
...
\section{Données de présence-absence}

\subsection{Indices de similarité}
...
Deux autres indices sont mentionnés ici, celui de \textcite{Sokal1963} et celui d'\textcite{Ochiai1957} :
...
La fonction \code{dist.binary} du package \emph{ade4} calcule $\sqrt{D_n}$ pour 10 indices de similarité étudiés par Gower et Legendre (voir l'aide de la fonction). 
Le résultat est une demi-matrice de distances de classe \code{dist}.
...
La distribution des valeurs de l'indice de Jaccard $S_3=1-D_3^2$ est en figure~\ref{JaccardFig}
\verbfootnote{Code R pour la figure:
  <<JaccardCode, eval=FALSE>>=
  plot(density(1-DistJaccard^2))
  @
}. 
Le mode est proche de 0,5: la moitié des espèces sont communes entre deux carrés.

\figureMargin{JaccardFig}
{Distribution des indices de Jaccard calculés pour les 50 carrés de BCI deux à deux.}
{
  <<JaccardFig, echo=FALSE, results='hide', out.width='\\textwidth'>>=
  <<JaccardCode>>
  @
}
...
La distribution des valeurs de l'indice corrigé se trouve en figure~\ref{ChaoJaccardFig}
\verbfootnote{Code R pour la figure:
  <<ChaoJaccardCode, eval=FALSE>>=
  plot(density(ChaoJaccard, na.rm=TRUE, to=1))
  @
}. 

\figureMargin{ChaoJaccardFig}
{Distribution des indices de Jaccard corrigés du biais d'etimation pour les 50 carrés de BCI deux à deux.}
{
  <<ChaoJaccardFig, echo=FALSE, results='hide', out.width='\\textwidth'>>=
  <<ChaoJaccardCode>>
  @
}
...
\figureSC{JaccardPCOFig}
{Représentation des 50 carrés de BCI dans une analyse en coordonnées principale fondée sur les distances de Jaccard.}
{
  <<JaccardPCOFig, echo=FALSE, results='hide', out.width='\\textwidth'>>=
  <<JaccardPCOCode>>
  @
}

La représentation de l'analyse est en figure~\ref{JaccardPCOFig}
\verbfootnote{Code R pour la figure:
  <<JaccardPCOCode, eval=FALSE>>=
...
\section{Données d'abondance}

La distance la plus évidente entre communautés quand les effectifs des espèces sont connus est simplement la distance euclidienne, éventuellement normalisée par le nombre d'espèces (Dissimilarité $D_1$ de \textcite{Gower1986}).
Son problème principal est qu'elle est très sensible aux ordres de grandeurs relatifs des variables.

La dissimilarité de Steinhaus \autocite{Motyka1947}, plus connue sous le nom de Bray-Curtis \autocite{Odum1950, Bray1957} (voir l'analyse historique de \textcite[][note de bas de page, eq. 7.58]{Legendre2012}) est très utilisée (par défaut dans plusieurs analyses du package \emph{vegan} par exemple):
...
La dissimilarité de Ružička \autocite{Ruzicka1958} généralise celle de Jaccard :
...
La différence de pourcentage (\foreignlanguage{english}{\emph{percentage difference}}) généralise la dissimilarité de Sørensen:
...
La plupart des dissimilarités sont implémentées par la fonction \code{dist.ldc} du package \emph{adespatial}.



\section{Emboîtement et substitution}

L'indice de Sørensen et les indices similaires prennent en compte deux types de dissimilarité: l'emboîtement des composition spécifiques (\foreignlanguage{english}{\emph{nestedness}}) et la substitution d'espèces (\foreignlanguage{english}{\emph{turnover}}).
...
\figureFW{Podani2011}
{
Triangle du simplex Similarité-Substitution-Emboîtement.
(a) Similarité, emboîtement et substitution somment à 1 donc les trois composantes peuvent être représentées dans un triangle et combinées deux à deux. La dissimilarité est par exemple représentée par la proximité au côté gauche du triangle, somme de l'emboîtement et de la substitution (ses sommets adjacents) et complément à 1 de la dissimilarité (le sommet opposé).
(b) Les formules de calcul dont données pour la similarité de Jaccard.
Avec ces notations classiques, $a$ est le nombre d'espèces communes $s_{1,1}$, $b$ et $c$ sont les espèces propres à chaque communauté, $s_{0,1}$ et $s_{1,0}$, et $n$ est le nombre total d'espèces $a+b+c$.
La figure est issue de \cite[][fig.~2]{Podani2011}.
}
{
  \includegraphics[width=\maxwidth]{Podani2011}
}
...
\textcite{Podani2011} représentent cette égalité dans un triangle dont les sommets correspondent aux trois termes (Figure \ref{Podani2011}).
...
La variabilité des contributions $\xq{H}{q}{}{\beta}{i}$ des communautés à l'entropie $\beta$ augmente avec l'emboîtement, mais cette voie n'a pas été explorée.



\section{Données d'abondance et similarité des espèces}

\subsection{Dissimilarité minimale moyenne}
...
$\pi_{s(n)}$ désigne la contribution de l'espèce $s$ à l'indice de Hurlbert, déjà vue dans le cadre de l'originalité taxonomique (page~\pageref{sec:OrigTax}).
La contribution des espèces rares augmente avec le paramètre $n$ (Figure~\ref{HurlbertCFig}, page~\pageref{HurlbertCFig}).
...
La dissimilarité pondérée (W pour \foreignlanguage{english}{\emph{weighted}}) entre les deux communautés (C pour \foreignlanguage{english}{\emph{community}}) en est la moyenne entre les deux communautés:
\begin{equation}
  d_{\mathit{CW}(n)} = \frac{1}{2}\left(\sum_s{\pi_{s(n)} \min{d_{s,2}}} + \sum_t{\pi_{t(n)} \min{d_{t,1}}} \right).
\end{equation}

Sa version non pondérée (U pour \foreignlanguage{english}{\emph{unweighted}}) par les abondances est \autocite{Ricotta2008}
\begin{equation}
...
\subsection{Similarité moyenne entre individus}
...
Une matrice de similarité $\matrixsym{Z}$ est définie entre les paires d'espèces: $z_{s,t}$ est la similarité entre l'espèce $s$ et l'espèce $t$, comprise entre 0 et 1 de la même façon que pour le calcul de la diversité $\xq{D}{q}{\matrixsym{Z}}{}{}$ (page~\pageref{sec:LeinsterCobbold}). 
La matrice $\matrixsym{Z}$ doit de plus être semi-définie positive, ce qui implique que $\sqrt{1-\matrixsym{Z}}$ est euclidienne: de cette façon, les espèces peuvent être représentées graphiquement par une PCoA. 
Les fréquences des espèces dans les deux communautés sont $\vectorsym{p}$ et $\vectorsym{q}$. 
...
La dissimilarité entre communautés de Rao (page~\pageref{sec:RaoDisc}) peut être normalisée (en la divisant par sa valeur maximale pour qu'elle soit comprise entre 0 et 1) puis transformée en similarité (en prenant son complément à 1) pour obtenir
...
Dans ce dernier cas, et  si $\matrixsym{Z}$ est la matrice identité $\matrixsym{I}$ (la similarité de deux espèces différentes est nulle et $z_{s,s}=1$), $S_{\mathit{Sorensen}}$ se réduit à l'indice de Sørensen vu plus haut.
...
Il se réduit à l'indice de Jaccard original pour les données de présence-absence et $\matrixsym{Z} = \matrixsym{I}$. 
...
\section[Distance entre paires de communautés et diversité beta]{Distance entre paires de communautés et diversité $\beta$}

\subsection{Diversité de Simpson}

\figureSC{BCIPCAFig}
{Représentation des deux premiers axes factoriels de l'ACP des 50 carrés de BCI. 
  Les données sont les probabilités des espèce, ni centrées ni réduites pour l'ACP. 
  Les carrés sont représentés avec les espèces. 
  Les carrés 35 et 40 ont une diversité faible et sont associés aux espèces \emph{Gustavia superba} et \emph{Alseis blackiana}.}
{
  <<BCIPCAFig, echo=FALSE, results='hide', out.width='\\textwidth'>>=
  <<BCIPCACode>>
  @
}
...
Cette \og définition\fg{} est valide puisque cette distance est la composante inter de la décomposition de l'entropie $\beta$ de Simpson (page~\pageref{eq:SimpsonNei}).

L'ACP non centrée des 50 carrés de BCI est en figure~\ref{BCIPCAFig}
\verbfootnote{Code R pour la figure:
  <<BCIPCACode, eval=FALSE>>=
...
\figureSC{BCIPCA2Fig}
{ACP centrée des 50 carrés de BCI pour visualiser la diversité $\beta$.}
{
  <<BCIPCA2Fig, echo=FALSE, results='hide', out.width='\\textwidth'>>=
  <<BCIPCA2Code>>
  @
}

L'ACP centrée (mais non réduite, figure~\ref{BCIPCA2Fig})
\verbfootnote{Code R:
  <<BCIPCA2Code, eval=FALSE>>=
  pcaBCI <- dudi.pca(BCI/rowSums(BCI), scannf = FALSE, nf = 2, center = TRUE, scale = FALSE)
  scatter(pcaBCI, clab.col = 0)
  @
} permet de mieux représenter la diversité $\beta$ : la distance de chaque carré au centre du repère est sa contribution.
...
\figureMargin{CordeFig}
{Distance de corde entre deux communautés représentées dans l'espace engendré par deux espèces.
  La distance $d_{1,2}$ entre les deux communautés est la longueur de la corde qui relie leurs vecteurs de composition normalisés sur le cercle de rayon 1.
  C'est aussi deux fois le sinus de l'angle $\nicefrac{\alpha_{1,2}}{2}$ formé par les deux vecteurs de composition des communautés.
}
{
  \includegraphics[width=\maxwidth]{Corde}
}

La distance de corde (Figure \ref{CordeFig}) est obtenue par la transformation
...
La distance de Hellinger est obbtenue par
\begin{equation}
  y_{s|i} = \sqrt{p_{s|i}}.
  \label{eq:Hellinger}
\end{equation}
...
\textcite[][annexe S3]{Legendre2013a} ont établi une liste de bonnes propriétés que doivent respecter les mesures de distance entre communautés pour analyser la diversité de différentiation.
La fonction \code{dist.ldc} du package \emph{adespatial} permet de calculer toutes les distances présentées dans l'article.
...
La diversité $\beta$ exprimée de cette façon peut faire l'objet d'analyses par les méthodes d'ordination classiques (RDA: Analyse de Redondance \autocite{Rao2014}; CCA: Analyse Canonique de Correspondance \autocite{Braak1986}) pour la relier à des variables environnementales par exemple\autocite{Legendre2013a}.

\figureSC{BCICordeFig}
{PCoA appliquée à la distance de corde des 50 carrés de BCI pour visualiser la diversité $\beta$.}
{
  <<BCICordeFig, echo=FALSE, message=FALSE, results='hide', out.width='\\textwidth'>>=
  <<BCICordeCode>>
  @
}

La diversité $\beta$ de BCI (Figure~\ref{BCIPCA2Fig}) est mieux représentée par une analyse en coordonnées principales appliquée à la distance de corde entre les communautés (Figure~\ref{BCICordeFig})
\verbfootnote{Code R:
  <<BCICordeCode, eval=FALSE>>=
  library("adespatial")
...
\section{Tests de significativité}
\label{sec:TestBetaDiff}

\subsection{Test de non-nullité}
...
\subsection{Analyse de la variabilité}
...
La fonction \code{betadisper} du package \emph{vegan} implémente ce test.
...
\chapter{Décomposition de l'entropie HCDT}
\label{chap:DedompHCDT}
...
\lettrine{C}{e chapitre} traite de la diversité proportionnelle, qui mesure à quel point les communautés diffèrent de leur assemblage.
...
Un forum a été consacré à la question dans la revue \emph{Ecology}, conclu par \textcite{Chao2012a}, et une revue a été faite par \textcite{Tuomisto2010a, Tuomisto2010b}.
...
\section{Décomposition de l'indice de Shannon}

\tableFW{table:Shannon}
  {Définition explicite de la diversité de Shannon en tant que divergence entre une distribution observée et une distribution attendue. 
Les indices $H_\alpha$ et $H_\gamma$ sont les compléments au logarithme du nombre d'espèces de cette divergence.
  }
  {
...
$H_\alpha$
  ...
  & $_{i}H_\alpha = \sum_s{p_{s|i}\ln{p_{s|i}}}$ \newline
    $H_\alpha = \sum_i{w_i\,_{i}H_\alpha}$ \\

$H_\beta$
  & Fréquence des espèces dans la communauté
  & Fréquence totale des espèces
  & $_{i}H_\beta = \sum_s{p_{s|i}\ln{p_{s|i}/p_{s}}}$ \newline
    $H_\beta = \sum_i{w_i\,_{i}{H}_\beta}$ \\
...
La décomposition explicite (Tableau~\ref{table:Shannon}) est due à \textcite{Marcon2012a} mais avait été établie plus ou moins clairement plusieurs fois dans la littérature, notamment par \textcite{Rao1985} : la diversité $\beta$ est la divergence de Kullback-Leibler entre la distribution des espèces dans chaque parcelle et la distribution moyenne, appelée divergence de Jensen-Shannon \autocite{Lin1991}. 
...
Calcul sous R: La fonction \code{BetaEntropy} calcule l'entropie $\beta$ de la méta-communauté (l'argument $q=1$ permet de choisir l'entropie de Shannon). 
...
Les données sont organisées comme dans le tableau~\ref{table:Notations}, mais les probabilités qu'un individu appartienne à la communauté $i$ et à l'espèce $s$ sont calculées pour que leur somme soit égale à 1 sur l'ensemble du tableau: $\sum_s{\sum_i{_{s,i}}}=1$ (au lieu de $\sum_i{p_{s|i}}=1$).
...
Cette correction est incluse dans la fonction \code{ShannonBeta} de \emph{entropart} :
<<bcShannonBeta>>=
...
Le meilleur estimateur peut être évalué à partir de la différence entre les entropies $\gamma$ et $\alpha$, corrigées par le meilleur estimateur disponible (celui de Chao, Wang et Jost, voir page~\pageref{sec:BiaisShannon}):
...
\begin{itemize}
  \item  La population totale étant partitionnée, chaque partition recevant un poids $w_i$, la composante intra-groupe de la mesure $H_{\alpha}$ est égale à la somme pondérée des mesures dans chaque-groupe $H_{\alpha}=\sum_i{w_i\xq[\,]{H}{}{}{\alpha}{i}}$;
  \item  La composante intergroupe $H_{\beta}$ est la mesure d'inégalité entre les groupes;
  \item  La mesure totale $H_{\gamma}$ est la somme des mesures intra et intergroupes.
\end{itemize}
...
\begin{equation}
  \xq{H}{q}{}{\gamma}{} = {\xq{H}{q}{}{\alpha}{}}+\ln_q{\xq{D}{q}{}{\beta}{}}-\left(q-1\right){\xq{H}{q}{}{\alpha}{}}\ln_q{\xq{D}{q}{}{\beta}{}}.
\label{eq:DecompJost}
\end{equation}

Ceci est la décomposition de \textcite{Jost2007}. 
Jost désigne sous le nom de \og composante $\beta$ de l'entropie\fg{} le logarithme d'ordre $q$ de la diversité, qu'il note $H_B$. 
La décomposition n'est pas additive, mais $H_B$ est indépendant de l'entropie $\alpha$.

Les deux derniers termes peuvent être regroupés et réarrangés pour obtenir $\xq{H}{q}{}{\beta}{}$ conformément à la décomposition additive de l'équation (\ref{eq:DecompHq}):

\begin{equation}
\label{eq:ContributionBeta}
  \xq{H}{q}{}{\beta}{} = \sum_i{w_i\sum_s{p^q_{s|i}\ln_q\frac{p_{s|i}}{p_s}}}.
\end{equation}

L'entropie $\beta$ est la somme pondérée par $w_i$ (et pas autrement) des contributions de chaque communauté:

\begin{equation}
  \xq{H}{q}{}{\beta}{i}=\sum_s{p^q_{s|i}\ln_q\frac{p_{s|i}}{p_s}}.
\end{equation}

$\xq{H}{q}{}{\beta}{i}$ est une divergence de Kullback-Leibler généralisée \autocite{Borland1998, Tsallis1998}.
$\xq{H}{q}{}{\beta}{}$ en est la moyenne sur l'ensemble des communautés, c'est donc une généralisation de la divergence de Jensen-Shannon \autocite{Marcon2014a}.
$\xq{H}{q}{}{\gamma}{}$ peut s'écrire sous la forme $\xq{H}{q}{}{\gamma}{} = -\sum_s{p^q_s}\ln_q{p_s}$: la formalisation de la décomposition est une généralisation de la décomposition de l'entropie de Shannon, résumée dans le Tableau~\ref{Decomposition}.

Il est intéressant d'écrire $\xq{H}{q}{}{\beta}{i}$ et $\xq{H}{q}{}{\gamma}{}$ sous la forme d'entropies, pour faire apparaître leur fonction d'information. 
...
Sa fonction d'information a été tracée Figure~\ref{IFig}, page~\pageref{IFig}.
...
La forme de l'entropie $\beta$ évoque la divergence de puissance \autocite{Cressie1984} qui est une statistique qui mesure la divergence entre deux distributions (déjà mentionnée page~\pageref{CressieRead}), dont les propriétés asymptotiques sont connues. 
La divergence de puissance peut être écrite sous la forme $I_q= \nicefrac{-2}{q} \sum_s{p_{s|i} \ln_q\nicefrac{p_{s}}{p_{s|i}}}$. 
...
La divergence non-logarithmique de Jensen-Shannon \autocite{Lamberti2003} est identique à une normalisation près: $K_q= -\sum_s{p_{s|i} \ln_q\nicefrac{p_{s}}{p_{s|i}}}$.
...
Calcul sous R: La fonction \code{DivPart} calcule entropie et diversité en même temps:

<<DivPartq>>=
DivPart(q = 1, Paracou618.MC, Biased = FALSE)
@

\section{Décomposition du nombre d'espèces}

Les résultats généraux se simplifient pour $q=0$. 
L'entropie $\gamma$ est le nombre d'espèces de la méta-communauté moins un ($\xq{H}{0}{}{\gamma}{}=S-1$), l'entropie $\alpha$ est la moyenne pondérée du nombre d'espèces des communautés moins un ($\xq{H}{0}{}{\alpha}{}=\bar{S}-1$), l'entropie $\beta$ est la différence: $\xq{H}{0}{}{\beta}{}=S-\bar{S}$.

En termes de diversité,

\begin{equation}
  \xq{D}{0}{}{\beta}{}=\frac{S}{\bar{S}}.
  \label{eq:D0beta}
\end{equation}

$\xq{D}{0}{}{\beta}{}$ peut être supérieur au nombre de communautés \autocite{Chao2012a} si les poids sont très inégaux, ce qui complique son interprétation. 
La pondération de Jost donne pour le nombre d'espèces le même poids à toutes les communautés et garantit $\xq{D}{0}{}{\beta}{}\le I$, ce qui constitue pour Chao et al. un argument en faveur de son utilisation. 
...
La communauté dont le poids $w_{min}$ est le plus faible est représentée une seule fois, les autres $\nicefrac{w_i}{w_{min}}$ fois.
L'indépendance entre les diversités $\alpha$ et $\beta$ est donc bien vérifiée.
Le diversité $\beta$ maximale théorique est $\nicefrac{1}{w_{min}}$ (et non $I$).
...
C'est précisément la définition de $\xq{D}{0}{}{\beta}{}$ (équation \ref{eq:D0beta}).


\section{Décomposition de l'indice de Gini-Simpson}
...
L'entropie $E_{\beta}$ calculée de cette manière est égale à $\xq{H}{2}{}{\beta}{}$ calculée à l'équation (\ref{eq:Hqbeta}) pour le cas général, mais la contribution de chaque communauté est différente: la décomposition de la variance est une façon alternative de décomposer l'entropie, valable uniquement pour l'entropie de Simpson.

Cette décomposition est remise en cause par \textcite{Jost2007, Jost2008}, qui a généré le débat \autocite{Heller2009, Ryman2009, Jost2009a, Whitlock2011}. 
$E_{\gamma}$ étant inférieure ou égale à 1, $E_{\beta}$ n'est pas indépendante de $E_{\alpha}$: seule la décomposition multiplicative permet l'indépendance, et Jost propose d'utiliser une transformation de $\xq{D}{2}{}{\beta}{}$ comme mesure de différenciation (voir page~\pageref{sec:Chevauchement}). 
\textcite{Gregorius2014} montre que ce problème n'est pas limité à l'entropie de Simpson mais s'applique à toutes les entropies HCDT d'ordre supérieur à 1.

Jost postule que la diversité $\beta$ de Simpson a la même relation à $E_{\beta}$ que les diversités $\alpha$ et $\gamma$,

\begin{equation}
  \xq{D}{q}{}{\gamma}{} = {\xq{D}{q}{}{\alpha}{}}{\xq{D}{q}{}{\beta}{}} \Leftrightarrow \frac{1}{1-E_{\gamma}}=\frac{1}{1-E_{\alpha}}\frac{1}{1-E_{\beta}},
\end{equation}
...
\tableFW{Decomposition}
  {Entropie, diversité et décomposition. 
L'entropie $\alpha$ de chaque communauté se calcule comme l'entropie $\gamma$}
  {
...
\midrule
Entropie $\gamma$ & 
	$\xq{H}{q}{}{\gamma}{}=-\sum_s{p^q_s}\ln_q{p_s}$ & 
	$\xq{H}{1}{}{\gamma}{}=-\sum_s{p_s}\ln{p_s}$ \\ \noalign{\medskip}

Entropie $\beta$ & 
	$\xq{H}{q}{}{\beta}{}=\sum_i{w_i\sum_s{p_{s|i}\left(\ln_q\frac{1}{p_s}-\ln_q\frac{1}{p_{s|i}}\right)}}$ &
	$\xq{H}{1}{}{\beta}{}=\sum_i{w_i\sum_s{p_{s|i}\ln\frac{p_{s|i}}{p_s}}}$ \\ \noalign{\medskip}

Diversité $\gamma$ (Nombre de Hill) & 
	$\xq{D}{q}{}{\gamma}{}=e^{^q\!H_{\gamma}}_q$ & 
	$\xq{D}{1}{}{\gamma}{}=e^{^1\!H_\gamma}$ \\ \noalign{\medskip}

Diversité $\beta$ (nombre équivalent) &
	$\xq{D}{q}{}{\beta}{}=e^{\frac{\xq{H}{q}{}{\beta}{}}{1-(q-1)\xq{H}{q}{}{\alpha}{}}}_q$ &
	$\xq{D}{1}{}{\beta}{}=e^{\xq{H}{1}{}{\beta}{}}$ \\
  
\bottomrule
\end{tabularx}
  }

Le nombre équivalent de communautés a une forme légèrement différente d'un nombre de Hill (Tableau~\ref{Decomposition}).

\section{Décomposition de l'indice de Rao}
\label{sec:RaoDisc}

Les notations sont celles de la présentation de l'indice de Rao, page~\pageref{sec:Rao}.
...
\begin{equation}
D_{H_{\Delta }}\left(\vectorsym{p}_i,\vectorsym{p}_j\right)
= 2 H_{\Delta}\left(\frac{\vectorsym{p}_{i}+\vectorsym{p}_j}{2}\right)-H_{\Delta}\left(\vectorsym{p}_j\right)-H_{\Delta}\left(\vectorsym{p}_j\right).
\end{equation}
...
\begin{equation}
H_{\Delta }\left(\vectorsym{p}\right) = \sum_i{w_i H_{\Delta }\left({\vectorsym{p}}_i\right)}+\sum_i{\sum_j{w_i w_j D_{H_{\Delta }}\left({\vectorsym{p}}_i,{\vectorsym{p}}_j\right)}}.
\label{eq:Raobeta}
\end{equation}

Le deuxième terme de la somme est la diversité inter-\break
communautés. 
...
L'exemple suivant utilise les communautés du fichier\break
\code{rao.effectifs.csv} :
...
\begin{itemize}
  \item  $H_{\Delta }\left(\vectorsym{p}\right)$: diversité totale. 
Calculer $\vectorsym{p}$ en sommant les communautés. 
Calculer ensuite l'entropie quadratique avec \code{divc} :
\end{itemize}
...
\begin{itemize}
  \item  $H_{\Delta}\left({\vectorsym{p}}_i\right)$: diversité de chaque communauté, fournie par \code{divc} :
\end{itemize}
...
\begin{itemize}
  \item  La variabilité inter-communautés peut être calculée par \code{divc} en utilisant la distance fournie par \code{disc}, en définissant la matrice des poids (égale aux effectifs):
\end{itemize}
...
La fonction \code{disc} calcule $\sqrt{2D_{H_{\Delta}}\left({\vectorsym{p}}_i,{\vectorsym{p}}_j\right)}$:
...
Les espèces peuvent être placées dans un espace multidimensionnel construit par une Analyse en Coordonnées Principales \autocite{Gower1966} de la matrice de dissimilarité $\Delta$ (les distances dans l'espace multidimensionnel sont $\sqrt{2d_{s's''}}$; autrement dit: la dissimilarité entre deux espèces est la moitié du carré de la distance entre elles dans la représentation géométrique).
...
\textcite{Hardy2008} montrent que les deux pondérations sont valides mais l'absence de cadre général assurant que la diversité $\gamma$ de Rao est supérieure à la diversité $\alpha$ \autocite{DeBello2010} motive \textcite{Guiasu2011} à proposer une alternative à l'indice de Rao, l'indice de Gini-Simpson quadratique pondéré (page~\pageref{eq:Guiasu}), dont la concavité est démontrée (ce qui implique que $\gamma\ge \alpha$). 
...
Ce résultat a été obtenu pour les arbres ultramétriques dans le cadre plus général de la décomposition de l'entropie phylogénétique (voir chapitre~\ref{chap:DedompHCDT} page~\pageref{chap:DedompHCDT}).

La partition peut être faite avec la fonction \code{DivPart} :
...
Les formules d'entropie et de diversité sont résumées dans le Tableau~\ref{Decomposition}.
...
La mesure de diversité $\xq{D}{q}{}{\beta}{}$ est le nombre équivalent de communautés totalement distinctes qui fourniraient ce niveau de diversité.


\subsection[Nécessité de normaliser la diversité beta]{Nécessité de normaliser la diversité $\beta$}
...
Pour une taille d'échantillon fixée (par exemple un hectare de forêt), la diversité $\beta$ ne dépend pas du nombre de communautés: l'entropie $\beta$ est l'espérance (estimée par la moyenne, équation~\ref{eq:ContributionBeta}, page~\pageref{eq:ContributionBeta}) de la divergence de Jensen-Shannon entre la distribution des espèces d'une communauté et celle de la méta-communauté, fixe. 
...
L'entropie $\beta$ entre les échantillons de deux hectares est donc diminuée de cette entropie $\beta$ \og intra\fg{} qui est absorbée par la nouvelle entropie $\alpha$ quand la taille des échantillons augmente.

La Figure~\ref{BetaSampleFig}
\verbfootnote{Code pour réaliser la figure:
  <<BetaSampleCode1, eval=FALSE, tidy=FALSE>>=
  library("gplots")
  ...
  BetaPlots(q = 2, nPlots=c(2, 5, 10, 15, 20, 25, 30), 
            nSimulations = 100)  
  @
} présente les valeurs estimées de diversité neutre $\beta$ entre carrés de BCI tirés aléatoirement, en fonction du nombre de carrés.

\figureSC{BetaSampleFig}
  {Estimation de la diversité $\beta$ entre carrés de BCI. 
La diversité est calculée à partir du nombre de carrés en abscisse, de 2 à 30, tirés aléatoirement. 
La valeur en ordonnée est la diversité d'ordre 2 moyenne sur 100 tirages, les barres représentent l'écart-type. 
Aux erreurs d'estimation près, la valeur de la diversité $\beta$ ne dépend pas du nombre de carrés.}
  {
  <<BetaSampleFig1, echo=FALSE, results='hide', message=FALSE>>=
  <<BetaSampleCode1>>
  @
  }
...
\subsection[L'indice de chevauchement CqN]{L'indice de chevauchement $C_{qN}$}
...
\begin{equation}
  C_{qN} = {\left[{\left(\frac{1}{\xq{D}{q}{}{\beta}{}}\right)}^{q-1}-{\left(\frac{1}{N}\right)}^{q-1}\right]}/{\left[1-{\left(\frac{1}{N}\right)}^{q-1}\right]}
\end{equation}
...
\chapter{Décomposition de la diversité phylogénétique et fonctionnelle}
...
\lettrine{L}{'entropie} phylogénétique est une combinaison linéaire de l'entropie HCDT donc sa décomposition est identique \autocite{Marcon2014b}. 



\section{Décomposition de la diversité phylogénétique}

En combinant l'équation (\ref{eq:Hbarq}) et le Tableau~\ref{Decomposition}, on obtient

\begin{equation}
  \xq{\bar{H}}{q}{}{\gamma}{}\left(T\right)
  = \xq[\,]{\bar{H}}{q}{}{\alpha}{}\left(T\right)
  + \xq[\,]{\bar{H}}{q}{}{\beta}{}\left(T\right).
\end{equation}
...
\begin{equation}
  \xq{\bar{H}}{q}{}{\alpha}{}\left(T\right)
  = \sum_i{w_i\xq[\,]{\bar{H}}{q}{}{\alpha}{i}\left(T\right)}.
\end{equation}
...
\begin{align}
\xq{\bar{H}}{q}{}{\beta}{}\left(T\right)
&= \sum_i{w_i}\xq[\,]{\bar{H}}{q}{}{\beta}{i}\left(T\right)\\
&= \sum_i{w_i}\sum_k{\frac{T_k}{T}\xq[\,]{H}{q}{}{\beta}{ik}}\\
&= \sum_i{w_i}\sum_k{\frac{T_k}{T}\sum_u{p^{q}_{k,u,i}\ln_q\frac{p_{k,u,i}}{p_{k,u}}}}
\end{align}

où $p_{k,u}$ est la probabilité d'observer une des espèces de la branche $u$ de l'arbre à la période $k$ (Figure~\ref{Arbre}) dans la méta-communauté, et $p_{k,u,i}$ la même probabilité dans la communauté $i$.

\figureSC{DivPartFig}
  {Représentation graphique de la diversité des deux parcelles du jeu de données \code{Paracou618}. 
Les rectangles transparents représentent la diversité neutre, les rectangles hachurés la diversité phylogénétique (l'arbre est la taxonomie des espèces, $q=1$). 
Dans chaque cas, le rectangle le plus large, de hauteur 1, représente la diversité $\gamma$. 
Le rectangle plus haut a pour largeur et longueur les diversités $\alpha$ et $\beta$. 
Les surfaces des deux rectangles sont identiques.}
  {
  <<DivPartFig, echo=FALSE, results='hide'>>=
  <<DivPartCode>>
  @
  }

La décomposition de l'entropie phylogénétique (au sens d'\textcite{Allen2009}), c'est-à-dire dans la cas particulier $q=1$, a été établie par \textcite{Mouchet2011}.

La fonction \code{DivPart} de \emph{entropart} permet de décomposer l'entropie. 
...
La Figure~\ref{DivPartFig}
\verbfootnote{Code pour réaliser la figure:
  <<DivPartCode, eval=FALSE, tidy=FALSE>>=
  data(Paracou618)
  q <- 1
  ...
  rect(0, 0, Phylo$TotalAlphaDiversity, Phylo$TotalBetaDiversity,
       density = 10)
  @
} donne une représentation graphique de la décomposition de la diversité. 
...
\section{Partitionnement de la diversité de Leinster et Cobbold}
...
\begin{equation}
  \xq{H}{q}{\matrixsym{Z}}{\alpha}{i}
  = \frac{1-\sum_s{p_{s|i}{\left(\vectorsym{Zp}\right)}^{q-1}_{s|i}}}{q-1}.
\end{equation}

L'entropie $\alpha$ est la somme pondérée des entropies des communautés:

\begin{equation}
  \xq{H}{q}{\matrixsym{Z}}{\alpha}{}
  = \sum_i{w_i\xq[\,]{H}{q}{\matrixsym{Z}}{\alpha}{i}}
  = \frac{1-\sum_i{w_i\sum_s{p_{s|i}{\left(\vectorsym{Zp}\right)}^{q-1}_{s|i}}}}{q-1}.
\end{equation}

L'entropie $\beta$ est similaire à l'entropie $\beta$ HCDT: c'est la divergence généralisée de Jensen-Shannon entre les distributions de $\vectorsym{Zp}$ des communautés. 
Formellement:

\begin{equation}
  \xq{H}{q}{\matrixsym{Z}}{\beta}{}
=\sum_i{w_i}\sum_{s}{p_{s|i}\left(\ln_q\frac{1}{{\left(\vectorsym{Zp}\right)}_s}-\ln_q\frac{1}{{\left(\vectorsym{Zp}\right)}_{s|i}}\right)}.
\end{equation}

La diversité $\beta$ est obtenue en calculant l'exponentielle déformée de la décomposition additive de l'entropie:

\begin{equation}
  \xq{D}{q}{\matrixsym{Z}}{\beta}{}
=e^{\frac{\xq{H}{q}{\matrixsym{Z}}{}{}}{1+\left(1-q\right)\xq[\,]{H}{q}{\matrixsym{Z}}{\alpha}{}}}_q.
\end{equation}

La décomposition de la diversité peut être faite directement, sans passer par l'entropie.

L'inverse de $\xq{D}{q}{\matrixsym{Z}}{\gamma}{}$ est la moyenne généralisée d'ordre $q-1$ de ${\left(\vectorsym{Zp}\right)}_s$:

\begin{equation}
  \frac{1}{\xq{D}{q}{\matrixsym{Z}}{\gamma}{}}
={\left[\sum_i{p_s{{\left(\vectorsym{Zp}\right)}_s}^{q-1}}\right]}^{\frac{1}{q-1}}
\end{equation}

Partant de $\xq{D}{q}{}{\gamma}{} = {\xq{D}{q}{}{\alpha}{}} {\xq{D}{q}{}{\beta}{}}$ et de la définition de Routledge de la diversité $\alpha$, on obtient

\begin{equation}
  \xq{D}{q}{\matrixsym{Z}}{\beta}{}
={\left[\sum_i{w_i}{\left(\frac{1/\xq[\,]{D}{q}{\matrixsym{Z}}{\alpha}{i}}{1/\xq[\,]{D}{q}{\matrixsym{Z}}{\gamma}{}}\right)}^{q-1}\right]}^{\frac{1}{q-1}}.
\label{eq:Dbeta}
\end{equation}

De la même façon que $1/\xq[\,]{D}{q}{\matrixsym{Z}}{\gamma}{}$ est la banalité moyenne des espèces, $\xq{D}{q}{\matrixsym{Z}}{\beta}{}$ est la banalité moyenne normalisée des communautés, où la banalité normalisée de la communauté $i$ est définie comme $\nicefrac{\left(1/\xq[\,]{D}{q}{\matrixsym{Z}}{\alpha}{i}\right)}{\left(1/\xq[\,]{D}{q}{\matrixsym{Z}}{\gamma}{}\right)}$, c'est-à-dire la banalité moyenne de ses espèces divisée par celle de la méta-communauté.

La fonction \code{DivPart} (comme les autres fonctions avancées de \emph{entropart}) supporte la décomposition de la diversité de Leinster et Cobbold quand une matrice de similarité lui est passée comme argument à la place d'un arbre phylogénétique pour le calcul de la phylodiversité HCDT:
...
Le partitionnement de la diversité fonctionnelle de la méta-communauté \code{Paracou618.MC} selon la matrice de distances des traits fonctionnels montre que la diversité $\beta$ est extrêmement faible entre les deux parcelles.



\chapter{Tests de significativité}
...
\lettrine{L}{a diversité} $\beta$ calculée entre plusieurs échantillons successifs d'une même communauté n'est pas nulle: par simple fluctuation des effectifs due au hasard, les distributions des échantillons ne sont pas identiques.
Des tests statistiques ont été développés pour rejeter cette hypothèse nulle à partir de données d'inventaires.

Ce chapitre ne traite pas de la diversité de différentiation: les tests correspondants ont été présentés page~\pageref{sec:TestBetaDiff}.

\section{Test de non-nullité}
...
\begin{itemize}
  \item  Les effectifs de chaque communauté $i$ sont tirés dans une loi multinomiale ${\mathcal M}(n_{+i},{n_{s,i}}/{n_{+i}})$ et ${\tilde{H}}_{\beta}$ est estimé;
  \item  La simulation est répétée un grand nombre de fois, par exemple 10000, et les valeurs extrêmes sont éliminées. Au seuil de risque $\alpha=5\%$, les 251\ieme\ et 9750\ieme\ valeurs simulées définissent les bornes de l'intervalle de confiance de l'hypothèse nulle.
\end{itemize}
...
Calcul sous R: la fonction \code{DivEst} produit un graphique avec l'intervalle de confiance (Figure~\ref{DivestFig}
\verbfootnote{
  Code R:
  <<DivestCode, eval=FALSE>>=
...
\figureSC{DivestFig}
{Estimation de la diversité de la méta-communauté \code{Paracou618}.}
{
  <<DivestFig, echo=FALSE, results='hide'>>=
  <<DivestCode>>
  @
}

Par défaut, \code{DivEst} ne corrige pas les biais d'estimation. 
...
\subsection{Correction des biais}
...
\section{Analyse de la variabilité}
...
\chapter{Autres approches}

\lettrine{D}{es définitions} différentes de la diversité $\beta$ ou du partitionnement de la diversité sont présentées ici parce qu'elles sont importantes dans la littérature même si elles n'entrent pas dans le cadre précédent.


\section{Le partitionnement de la diversité selon Pélissier et Couteron}

\subsection{Cadre}

\tableFW{Couteron}
  {Tableau d'occurrence des espèces.}
  {
    \begin{tabularx}{\textwidth}{l l l X c l}
    \toprule
      &  &  & Forêt $k$    & \dots & Total \\
      &  &  & Parcelle $j$ & \dots & \\
      &  &  & Placette $i$ & \dots & \\
    ...
    Famille $u$
      & Genre $t$ 
      & Espèce $s$ 
      & $n_{sh}=1$ si l'arbre $h$ appartient à l'espèce $s$, $0$ sinon.\newline $n_{s\cdot}={n_{s+}}/{n}$ 
      &  &  \\
...
Le Tableau~\ref{Couteron} est appelé tableau d'occurrence des espèces.
...
La mesure de dissimilarité moyenne entre les individus, notée $ADW_{\rho}$ pour \foreignlanguage{english}{\emph{Average Dissimilarity Within}}, où $\rho$ représente la dissimilarité choisie, est simplement l'entropie quadratique de Rao.
Si la dissimilarité n'est pas euclidienne, la concavité de $ADW_{\rho}$ n'est pas garantie, ce qui motive Kosman à proposer une autre mesure \autocite{Kosman1996, Kosman2007}, ${KW}_{\rho}$, qui est la dissimilarité moyenne des $n$ paires d'individus les plus dissimilaires (au lieu de toutes les $\nicefrac{n(n-1)}{2}$ paires).
