...
L'équivalence de ces deux concepts n'est pas évidente: elle a été montrée pour les mesures de dissimilarité entre paires de communautés par \textcite{Legendre1999, Legendre2001, Legendre2005} et pour l'entropie de Rao par \textcite{Pavoine2004} mais reste une question ouverte pour l'entropie HCDT en général.
...
\verbfootnote{Code R pour la figure:
  ...
  ggplot(data.frame(Jaccard=as.numeric(1-DistJaccard^2)), aes(x=Jaccard)) +
    geom_density()
  @
}. 
...
\verbfootnote{Code R pour la figure:
  ...
  ggplot(data.frame(ChaoJaccard=as.numeric(ChaoJaccard)), aes(x=ChaoJaccard)) +
    geom_density()
  @
}. 
...
La dissimilarité de Steinhaus \autocite{Motyka1947}, plus connue sous le nom de Bray-Curtis \autocite{Odum1950, Bray1957} (voir l'analyse historique de \textcite[][note de bas de page, eq. 7.58]{Legendre2012}), qui pourrait être attribuée à \autocite{Renkonen1938}, est très utilisée (par défaut dans plusieurs analyses du package \emph{vegan} par exemple):
...
\section[Distance entre paires de communautés et diversité beta proportionnelle]{Distance entre paires de communautés et diversité $\beta$ proportionnelle}
...
La diversité $\beta$ exprimée de cette façon peut faire l'objet d'analyses par les méthodes d'ordination classiques (RDA: Analyse de Redondance \autocite{Rao1964}; CCA: Analyse Canonique de Correspondance \autocite{Braak1986}) pour la relier à des variables environnementales par exemple\autocite{Legendre2013a}.
