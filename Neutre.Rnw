...
\verbfootnote{Le code R nécessaire pour réaliser la figure est:
  <<ShannonCode, eval=FALSE, tidy=FALSE>>=
...
\verbfootnote{
Code R:
<<ShannonBiaisCode, eval=FALSE, tidy=FALSE>>=
  Biais.p <- function (p) {
...
\verbfootnote{
Le code R nécessaire pour réaliser la figure est:
<<KullbackLeiblerCode, eval=FALSE, message=FALSE, fig.asp=1>>=
p <- q <- seq(0.01, 1, .01)
...
\verbfootnote{
  Code R :
  <<ProfilDivCode, eval=FALSE, tidy=FALSE>>=
  q.seq <- seq(0, 2, .1)
...
\verbfootnote{Code R pour réaliser la figure:
  <<comProfilzetaCode, eval=FALSE, tidy=FALSE>>=
  # Profil P6
...
\verbfootnote{Code R pour réaliser la figure:
  <<comProfilHCDTCode, eval=FALSE, tidy=FALSE>>=
...
\verbfootnote{Code R pour réaliser la figure:
  <<zetaDiffCode, eval=FALSE, tidy=FALSE>>=
...
\verbfootnote{Code R pour réaliser la figure:
  ...
  J <- H/log(S)
  ggplot(data.frame(S, J), aes(S, J)) +
    geom_point()
  @
  <<SJLogNormaleECode, eval=FALSE>>=
  E <- sapply(rcl, Simpson)
  ggplot(data.frame(S, E), aes(S, E)) +
    geom_point()
  @
  <<SJLogNormaleJ2Code, eval=FALSE>>=
  J2 <- E/(1-1/S)
  ggplot(data.frame(S, J2), aes(S, J2)) +
    geom_point()
  @
