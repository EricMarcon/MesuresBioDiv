...
$\vectorsym{p}\allowbreak=\left\{p_1,p_2,\dots,p_S\right\}$. 
...
Rényi pose également les axiomes pour une mesure d'entropie $R\left(\vectorsym{p}\right)$, où $\vectorsym{p}=\left\{p_1,p_2,\dots,p_S\right\}$:
\begin{itemize}
...
Cette dernière propriété sera traitée en détail au chapitre~\ref{chap:Beta}. 
...
Enfin, sa décomposition sera présentée en détail dans le chapitre~\ref{Decomposition}.
...
L'entropie est une moyenne pondérée par les probabilités de la fonction d'information, c'est donc une fonction linéaire des probabilités, propriété importante pour définir l'entropie $\alpha$ (chapitre~\ref{chap:Beta}) comme la moyenne des entropies de plusieurs communautés, ou l'entropie phylogénétique (chapitre~\ref{chap:Phylodiversite}) comme la moyenne de l'entropie sur les périodes d'un arbre. 
...
{Comparaison des profiles de diversité des parcelles 6 et 18 de Paracou avec leur intervalle de confiance à 95\%. La parcelle 18 est tracée en rouge, lignes continues, et la 6 en vert, lignes pointillées.}
{
