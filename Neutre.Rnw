...
\verbfootnote{
Code R:
<<BiaisJack3Code, eval=FALSE, tidy=FALSE>>=
ggplot(data.frame(n=n.seq, Biais, lm1=predict(lm1), 
...
\figureSC{NonObserve}
  {Nombre de nouvelles espèces découvertes en fonction de l'effort d'échantillonnage supplémentaire (données de BCI). 
...
\verbfootnote{Le code R nécessaire pour réaliser la figure est:
  ...
  ggplot(data.frame(x=c(1, 10000)), aes(x)) + stat_function(fun=Sprime) +
    labs(x = "n'", y="S'")
  @
}.
...
\verbfootnote{Le code R nécessaire pour réaliser la figure est:
  <<Lineweaver2, eval=FALSE, tidy = FALSE>>=
...
\verbfootnote{Le code R nécessaire pour réaliser la figure est:
  ...
  ggplot(data.frame(x, 
      ...
      aes(x)) +
    geom_point(data=data.frame(nArbres, nEspeces), mapping=aes(nArbres, nEspeces)) +
    geom_line(aes(y = MM)) +
