...
Le carré de la norme du vecteur en $S$ dimensions dont les coordonnées sont $(1-p_s)^{n/2}$ est $\sum_s{(1-p_s)^n}$, c'est-à-dire ${\mathbb E}(\xq{s}{}{n}{0}{})$, l'espérance du nombre d'espèces non observées.
Celui du vecteur de coordonnées $p_s (1-p_s)^{n/2-1}$ est $\sum_s{p_s^2(1-p_s)^{n-2}}=\nicefrac{2}{n(n-1)}{\mathbb E}(\xq{s}{}{n}{2}{})$.
Enfin, le produit scalaire des deux vecteurs vaut $\sum_s{p_s(1-p_s)^{n-1}}=\nicefrac{1}{n}{\mathbb E}(\xq{s}{}{n}{1}{})$.
...
\begin{equation} 
  \left[ \sum_s{(1-p_s)^n} \right] \left[ \sum_s{p_s^2(1-p_s)^{n-2}} \right] \ge \left[ \sum_s{p_s(1-p_s)^{n-1}} \right]^2
\end{equation}


D'où:
\begin{equation} 
 {\mathbb E}(\xq{s}{}{n}{0}{}) \ge \frac{n-1}{n}\frac{\left[ {\mathbb E}(\xq{s}{}{n}{1}{}) \right]^2}{2 {\mathbb E}(\xq{s}{}{n}{2}{})}
\end{equation}
...
\begin{equation}
  \Var{\left({\hat{S}}_\mathit{Chao1}\right)} = {\xq{s}{}{n}{2}{}}\left[\frac{1}{2}{\left(\frac{\xq{s}{}{n}{1}{}}{\xq{s}{}{n}{2}{}}\right)}^2 + {\left(\frac{\xq{s}{}{n}{1}{}}{\xq{s}{}{n}{2}{}}\right)}^3 + \frac{1}{4}{\left(\frac{\xq{s}{}{n}{1}{}}{\xq{s}{}{n}{2}{}}\right)}^4\right]
\end{equation}
...
L'estimateur jackknife n'a pas de support théorique solide \autocite{Cormack1989} mais est efficace en pratique, notamment parce que son ordre peut être adapté aux données.
...
On peut maintenant écrire le logarithme de $L$: $\ln{L}=\ln{n!}-\sum^S_{s=1}{\ln  {np_s}}$. 
On utilise l'approximation de Stirling, $\ln{n!}\approx n\ln{n}-n$, pour obtenir après simplifications:
...
$\zeta_{u,v}$ est la somme sur toutes les espèces de la probabilité de rencontrer $u$ fois l'espèce dans un échantillon de taille $u+v$. 
...
$\mathrm{\Psi}\left(n_s\right)$ est la fonction digamma.
...
library("sp")
image(xyz, col=bpy.colors(n = 100, cutoff.tails = 0.4, alpha = .7), 
      xlab="q", ylab="p")
...
\section{Entropie et biodiversité: historique}
...
\begin{eqnarray}
  y(x) = \sum_{s=x+1}^{S}{p_{[s]}},\ x\in \{0, 1, \ldots, S\}
\end{eqnarray}
...
La formule obtenue contient le développement en série entière de $[1 - (1-A)^{q-1}]$, ce qui permet d'éliminer la somme infinie et d'obtenir l'estimateur de l'équation~\eqref{eq:HqChaoJost} après simplifications \textcite[][Appendix S1, Theorem S1.2d]{Chao2015}.
...
\textcite{Ricotta2003b} généralise cette mesure à d'autres fonctions de rareté.
