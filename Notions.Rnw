...
Les indicateurs de la biodiversité peuvent englober bien d'autres choses que la diversité du vivant : le nombre d'espèces menacées (par exemple la liste rouge de l'IUCN), la taille des populations ou la surface des écosystèmes préservés, la dégradation des habitats, la menace pesant sur des espèces emblématiques\dots
...
La mesure de diversité est alors limitée à un taxocène, c'est-à-dire un sous-ensemble des espèces d'une communauté reliées taxonomiquement: les papillons, les mammifères, les arbres (la délimitation du sous-ensemble n'est pas forcément strictement taxonomique)\dots
...
On peut bien évidemment s'intéresser à d'autres niveaux et d'autres objets, par exemple la diversité génétique (en termes d'allèles différents pour certains gènes ou marqueurs) à l'intérieur d'une population. 
...
(on se référera par exemple à \textcite{Chapin2000} pour une revue sur cette question ou \textcite{Cardinale2012} pour les conséquences de l'érosion de la biodiversité sur les services écosystémiques). 
...
Le concept le plus répandu est celui d'espèce \emph{biologique} \autocite{Mayr1942, Dobzhansky1937} : un \og groupe de populations naturelles isolées reproductivement les unes des autres\fg{} \autocite{Mayr1970}. 
...
L'approche phylogénétique (page~\pageref{sec:Phyloentropie}) règle le problème : si deux taxons très semblables apportent à peine plus de diversité qu'un seul taxon, le choix de les distinguer ou non n'est pas critique.
...
\lettrine{Q}{uelques} outils sont nécessaires avant d'entrer dans le c\oe ur du sujet. Les relations décrivant le nombre d'espèces en fonction de la taille de l'échantillon (relations aire-espèces) et la distribution de l'abondance des espèces sont importants pour les écologues. La mesure de l'exhaustivité de l'échantillonnage par le taux de couverture sera la base de l'estimation de la diversité à partir de données réelles.
...
Les données sont souvent celles de la parcelle forestière permanente de Barro Colorado Island (BCI)\autocite{Condit2012}: \SI{50}{\hectare} de forêt tropicale dont les arbres de plus de \SI{1}{\centi\meter} de diamètre à hauteur de poitrine (DBH: \foreignlanguage{english}{\emph{Diameter at Breast Height}}) ont été inventoriés.
...
D'autres exemples utilisent des inventaires de la forêt de Paracou en Guyane française \autocite{Gourlet-Fleury2004}, principalement deux placettes d'un hectare numérotées 6 et 18, dont les données, toujours des arbres de plus de \SI{10}{\centi\meter} DBH, se trouvent dans le package \emph{entropart}.
...
\begin{itemize}
  ...
  La question traitée est la relation entre le nombre d'espèces et la taille de la communauté;
...
\textcite{Good1953} définit le taux de couverture (\foreignlanguage{english}{\emph{sample coverage}}) de l'échantillonnage comme la proportion des espèces découvertes $C=\sum^S_{s=1}{{\mathbf 1}\left(n_s>0\right)p_s}$ où ${\mathbf 1}(\cdot)$ est la fonction indicatrice. 
...
L'estimateur de Good néglige le terme $\pi_1$, la somme des probabilités des espèces observées une fois. Ce terme peut être estimé avec un biais plus petit.
\textcite{Chao1988} puis Zhang et Huang proposent l'estimateur suivant, qui utilise toute l'information disponible et a le plus petit biais possible:
...
Un autre estimateur est utilisé dans le logiciel SPADE \autocite{Chao2010a} et son portage sous R, le package \emph{spadeR} \cite{Chao2016c}.
