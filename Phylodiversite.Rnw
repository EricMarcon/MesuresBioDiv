...
Le calcul sous R est immédiat avec un arbre au format \code{phylog} du package \emph{ade4}.
Un arbre au format \code{phylo} du package \emph{ape} nécessite une conversion : la fonction \code{Preprocess.Tree} du package \emph{entropart} la réalise.
...
\verbfootnote{
  ...
  autoplot(CommunityProfile(Diversity, Rs), xlab="Ordre de diversité", ylab="Diversité") +
...
\verbfootnote{Le code R nécessaire pour réaliser la figure est:
  ...
  r <- 0.5
  seqZp <- seq(0, 1, 0.001)
  ggplot(data.frame(r=seqZp, xr=seqZp^r), aes(r, xr)) +
    geom_line() +
...
La transformation d'une matrice en un arbre phylogénétique déforme les données.  \autocite{Pavoine2005a}\autocite{Podani2006, Podani2007}  
