...
\verbfootnote{Le code R nécessaire pour réaliser la figure est:
  <<Beguinot2015aCode, evalNN=FALSE>>=
  ggplot() +
    geom_ribbon(aes(x, ymin = Min, ymax = Max), 
                data.frame(x=n:Nmax, Min, Max), alpha = 0.3) +
    geom_line(aes(x, y), data.frame(x=0:Nmax, y=c(0, SAC))) +
    geom_line(aes(x, y), data.frame(x=n:Nmax, y=Moyenne), col = "red", lty=2) +
    geom_line(aes(x, y), data.frame(x=nReduit:Nmax, y=MoyenneReduit), col = "blue", lty=3) +
    labs(x = "Nombre d'individus", y = "Nombre d'espèces")
  @
