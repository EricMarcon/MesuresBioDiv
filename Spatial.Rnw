...
\verbfootnote{Le code R nécessaire pour réaliser la figure est:
  <<gammaCode, eval=FALSE, tidy=FALSE>>=
  library("GoFKernel")
  k <- 0.2
  p <- 100
...
{
  <<gammaFig, echo=FALSE, results='hide', message=FALSE>>=
  <<gammaCode>>
  @
}
...
\verbfootnote{Le code R nécessaire pour réaliser la figure est:
  <<nbinCode, eval=FALSE, tidy=FALSE>>=
...
\figureSC{nbinFig}
{
  Réalisation d'une loi gamma de paramètres $0,2$ et 100. L'histogramme représente la proportion (qui estime la probabilité) des valeurs inférieures à 50. Les espèces d'abondance nulle ne sont pas observées dans l'échantillon.
}
{
...
\verbfootnote{Code R pour réaliser la figure:
  <<KemptonQCode, eval=FALSE, tidy=FALSE>>=
...
\verbfootnote{Code R:
  <<SACzetaCode, eval=FALSE, tidy=FALSE>>=
...
\verbfootnote{Le code R nécessaire pour réaliser la figure est:
  <<Beguinot2015Code, eval=FALSE, tidy=FALSE>>=
...
\verbfootnote{Le code R nécessaire pour réaliser la figure est:
  <<Beguinot2015aCode, eval=FALSE, tidy=FALSE>>=
  ggplot() +
